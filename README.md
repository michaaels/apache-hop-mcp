...
Version 0.6.0 adds bounded tabular property authoring to the **Apache Hop Native Semantic MCP**. MCP clients can now discover and populate repeated field definitions and similar one-level collections exposed by Hop's metadata-injection model, while retaining scalar authoring and the transactional write path. Inspection remains enabled by default; execution and writes require separate command-line flags.
...
The Marketplace artifact is `target/apache-hop-mcp-0.6.0.zip`, containing:

```text
plugins/misc/apache-hop-mcp/
  apache-hop-mcp-0.6.0.jar
  version.xml
  lib/...
```
...
After the `v0.6.0` GitHub Release exists, import `marketplace/hop-marketplace-repo.yaml` into Hop Marketplace and install **Apache Hop MCP**, or use:

```bash
./hop marketplace install io.github.michaaels:apache-hop-mcp:0.6.0 --repo apache-hop-mcp
```
