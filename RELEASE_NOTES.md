# Apache Hop MCP 0.5.0 — Native semantic component authoring

First release that can construct useful pipelines and workflows through Apache Hop's own Java model when explicitly authorized.
...
- Discovers installed transforms/actions and derives their safe scalar configuration schema from Hop's metadata-injection model.
- Adds configured native transforms/actions with `add_component` and composes them with hop operations in one transaction.
- Rejects unknown, collection-shaped, oversized and secret-looking component properties.
...
- GitHub Actions runs `mvn -B clean verify` and validates the Marketplace ZIP before creating `v0.5.0`.
